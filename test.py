...
def create_images ():
    ...
    A_true = plt.imread('pikacho-S.png')  # True underlying image
    ...
    for i in range(num_images):
        ...
        name = '/mnt/c/Users/noyga/PycharmProjects/SGD_Images/output/' + 'pikacho-S' + str(i) +'.mrc'
        X_list.append(
            apply_transformation(A_true, rotation, translation, 1) + sigma * np.random.randn(image_size, image_size))
        # plt.savefig(name)
        # # Save the estimated image as PNG
        # imageio.imwrite(name, (X_list[i] * 255).astype(np.uint8))
